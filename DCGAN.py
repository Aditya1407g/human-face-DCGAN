...
class DCGAN(object):
    ...
    def train_dcgan_lsgan_transfer(self, source_img_227, source_img_128, imgs, true_label_fea_128, true_label_fea_64,
        ...
        """
        Conditional least-squares GAN loss with feature and age losses.
        """
        ...
        self.d_loss = (0.5 * (d_loss_real + 0.5 * (d_loss_fake1 + d_loss_fake2))) * self.gan_loss_weight

        self.g_loss = (0.5 * tf.reduce_mean(tf.square(D3_logits - 1.))) * self.gan_loss_weight

        g_source_scaled = (self.g_source + 1.) * 127.5
        # Optionally add total variation loss:
        # self.tv_loss = total_variation_loss(g_source_scaled) * self.tv_loss_weight

        g_source_scaled = tf.image.resize_bilinear(images=g_source_scaled, size=[227, 227])
        g_source_scaled = g_source_scaled - self.mean

        self.face_age_alexnet(g_source_scaled, if_age=True, reuse=True)
        ...
        total_g_loss = self.g_loss + self.fea_loss + self.age_loss

        # Add TensorBoard summaries for monitoring
        tf.summary.scalar('d_loss', self.d_loss)
        tf.summary.scalar('g_loss', self.g_loss)
        tf.summary.scalar('fea_loss', self.fea_loss)
        ...
        g_optim = tf.train.AdamOptimizer(self.learning_rate, beta1=0.5).minimize(total_g_loss, var_list=self.g_vars)
